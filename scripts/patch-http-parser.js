/**
 * Patches websocket-driver so Vue CLI 4 works on Node 20+
 * (process.binding('http_parser') was removed).
 */
const fs = require('fs')
...
if (!fs.existsSync(target)) {
  console.warn('[patch-http-parser] websocket-driver not found, skipping')
  process.exit(0)
}
...
var NodeHTTPParser;
try {
  NodeHTTPParser = process.binding('http_parser').HTTPParser;
} catch (err) {
  NodeHTTPParser = require('http-parser-js').HTTPParser;
}

var version = NodeHTTPParser.RESPONSE ? 6 : 4;
...
console.log('[patch-http-parser] Patched websocket-driver for modern Node')
